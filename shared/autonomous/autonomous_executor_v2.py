...
class AutonomousExecutorV2:
    ...
    def __init__(
        ...
        self.use_real_execution = os.getenv('USE_REAL_EXECUTION', 'true').lower() == 'true'
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            # Initialize real executor if enabled
            if self.use_real_execution:
                try:
                    from autonomous.real_executor import RealExecutor
                    self.real_executor = RealExecutor(
                        workspace_path=self.project_path,
                        llm_api_key=os.getenv('LLM_API_KEY') or os.getenv('ANTHROPIC_API_KEY'),
                        llm_model=os.getenv('LLM_MODEL', 'claude-sonnet-4-20250514')
                    )
                    logger.info(f"Real executor initialized: {self.real_executor.get_status()}")
                except Exception as e:
                    logger.warning(f"Failed to initialize real executor: {e}. Falling back to mock.")
    ...
    async def _execute_single_task(self, task: TaskState) -> Dict[str, Any]:
        """
        Execute a single task with retry logic (REAL or MOCK)
        ...
        """
        try:
            if self.use_real_execution and self.real_executor:
                # REAL EXECUTION using OpenHands/LLM
                logger.info(f"[REAL EXECUTION] Task {task.task_id}")
                print(f"   🔥 REAL EXECUTION: {task.description}")
                
                # Convert TaskState to task dict for real_executor
                task_dict = {
                ...
                result = await self.real_executor.execute_task(task_dict)
                ...
                logger.info(f"[MOCK EXECUTION] Task {task.task_id}")
                print(f"   🎭 MOCK EXECUTION: {task.description}")
